...
if(isset($_GET['id']) && is_numeric($_GET['id'])) {
  ...
  $action = "update";
  $btnText = "Actualizar";
  $pageTitle = "Actualización de usuario";
  $pageName = "Actualización";
  $idUser = (int)$_GET['id'];
  $res = $db->getUserById($idUser);
  if($res->num_rows == 0) {
      ...
      <div class="col-lg-6 col-md-8 mx-auto">
        ...
        <form class="row g-3" id="<?=$frmId?>" method="post">
            ...
            <div class="col-md-6">
                ...
                <input type="text" class="form-control" name="name" id="inputName" value="<?=$userData->name?>" required minlength="2" maxlength="100">
            </div>
            <div class="col-md-6">
                <label for="inputSurname" class="form-label">Apellidos</label>
                <input type="text" class="form-control" name="surname" id="inputSurname" value="<?=$userData->surname?>">
            </div>
            <div class="col-md-9">
                <label for="inputMail" class="form-label">Email</label>
                <input type="email" class="form-control" name="email" id="inputMail" value="<?=$userData->email?>" required>
            </div>
            <div class="col-md-3">
                <label for="inputId" class="form-label">DNI</label>
                <input type="text" class="form-control" name="dni" id="inputId" value="<?=$userData->dni?>" required minlength="9" maxlength="9">
            </div>
            <div class="col-md-6">
                <label for="inputAddress" class="form-label">Dirección</label>
                <input type="text" class="form-control" name="address" id="inputAddress" value="<?=$userData->address?>" placeholder="">
            </div>
            <div class="col-md-6">
                <label for="inputPhone" class="form-label">Teléfono</label>
                <input type="number" class="form-control" name="phone" id="inputPhone" value="<?=$userData->phone?>" placeholder="">
            </div>
            <div class="col-12">
            <label for="inputRole" class="form-label">Rol</label>
            <select id="inputRole" class="form-select" name="role">
                <?php foreach(IbiSolution::ROLES as $role=>$roleEsp):
                    $selectedAttr = ($role == $userData->role) ? " selected" : '';
                    ?>
                    
                    <option value="<?=$role?>"<?=$selectedAttr?>><?=$roleEsp?></option>
                <?php endforeach;?>
            </select>
            </div>
